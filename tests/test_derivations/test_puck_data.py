"""Tests for gwifi puck identity derivation from WiFi-sheet extra columns."""
import pytest
...
def test_happy_path_attaches_puck_data():
    host = _host("puck04", extra={"#": "4", "Serial": "2831HW00VZA"})
    enrich_hosts_with_puck_data([host])
    assert host.puck_data == PuckData(number=4, serial="2831HW00VZA")
...
def test_number_outside_range_raises():
    ...
    with pytest.raises(ValueError, match="100"):
        enrich_hosts_with_puck_data([host2])
...
def test_duplicate_numbers_across_hosts_raise():
    ...
    with pytest.raises(ValueError, match="duplicate"):
        enrich_hosts_with_puck_data([host1, host2])
