...
- Clean-slate now also clears ALL borders: one `updateBorders` request over
  ...
  after `unmergeCells` and before any new formatting.
- `MERGE_COLUMNS_PER_BLOCK = ["Site", "Physical Location", "Hardware",
  ...
  row — are intentionally collapsed.)

### 2. `src/gdoc2netcfg/sources/parser.py` — Site carry-forward

A data row whose Site cell is blank inherits the Site of the immediately
preceding row **when that row has the same machine name** (contiguous block,
same sheet). First-row-blank stays blank (all-sites semantics unchanged for
genuinely site-less machines). This is the parser-side meaning of a merged
Site cell and also fixes the pre-existing ab-30 partial-filter bug. TDD.

### 3. `scripts/wifi-sheet-migrate.py` — populate additions

- `OPENMESH_SITE_BY_MACHINE`: ab-38, 96-00 → `welland`; ab-30, 94-98, 95-80,
  95-88 → `monarto`. Written into the Site column of the OpenMesh rows that
  populate emits (Physical Location text untouched).
...
- `tests/fixtures/wifi_sheet.csv` regenerated to the post-change published
  ...
  per the overlay, tenwrt row appended. Golden `pucks.json` byte-identity
  must still pass UNMODIFIED (12 pucks; tenwrt absent).
...
## Live rollout (after code lands on PR #18)

1. `populate` re-run (writes Site overlay + tenwrt, refreshes snapshot).
2. `wifi-sheet-format.py` run (clears stale borders, applies 6-column merges
   + correct block borders).
3. Verify: fresh `fetch`; `generate wifi --stdout` byte-identical to the
   deployed pucks.json; `wifi-sheet-scan.py` clean; welland `validate` error
   delta explained (monarto APs leaving welland shrinks the transitional
   error set).
