...
The `gwifi_pucks` generator (welland only) reads gale puck hosts from the "wifi" sheet (ordinary WiFi-sheet rows carrying `#`/`Serial` extras — see the `[sheets] wifi` entry) and emits `wisp/pucks.json`, the gale puck identity file for the netboot-install service on wisp.welland.mithis.com (deploy: `scp wisp/pucks.json wisp.welland.mithis.com:/tmp/ && ssh wisp.welland.mithis.com 'sudo install -m 0644 /tmp/pucks.json /etc/gwifi-netboot/pucks.json && sudo systemctl restart gwifi-netboot'`). Puck host-records (A+PTR) resolve site-wide through the normal `dnsmasq_internal` generator, since pucks are now ordinary inventory hosts. See `gwifi-openwrt` `docs/wisp-netboot-install-design.md`.
