...
**`discovery.db` is the sole source for supplement data.** Supplement scans take their last-known baseline from the DB and the CLI persists results back to the DB only — the flat supplement `.json` caches are neither read nor written. `_build_pipeline` and the show/publish commands load supplements via `load_latest_*` (a supplement with no completed scan contributes no enrichment). Scan freshness is the age of the latest completed scan per scan_type (`scans` table, 5-minute window) instead of file mtimes. `cmd_fetch` still writes the CSVs to both the flat cache (the parser input) and `config.db`. CSV snapshots are delta-based too: `save_csv` stores nothing when the text equals the sheet's latest finished snapshot (before 2026-09 every 15-minute fetch stored a full copy — 733 MB of which >99 % were duplicates; `scripts/dedupe_csv_snapshots.py --execute` collapsed the existing consecutive duplicates and VACUUMed). The DBs are created automatically on first write (daemon, scan commands, fetch); inspect with `db info` / `db history`.
...
|------|------|-----------|-------------|------------|
| welland | `ten64.welland.mithis.com` (10.1.10.1) | `10.1.X.X` | `2404:e80:a137:1XX::` | dnsmasq_leaf, pdns_internal, pdns_external, recursor_forward, nginx, known_hosts, wifi, rsyslog (+ nagios, currently unused) |
| monarto | `ten64.monarto.mithis.com` (10.2.10.1) | `10.2.X.X` | `2404:e80:a137:2XX::` | the same, minus nagios |
...
Both sites run the redesigned per-net-leaf + pdns architecture (recursor on 10.X.0.1, pdns@internal auth on 127.0.0.1:5300, pdns@external public, both DNSSEC-signed). Welland's external binds its public IPv4 (87.121.95.37); monarto is v6-primary — its external service/AXFR address is 2404:e80:a137:200::1 (no public IPv4). The differences (verified against both live configs 2026-09-12): `nagios` is enabled on welland only, and it is **unused** — nothing consumes its `nagios-switches.cfg` and no nagios is installed. Everything else is the same at both sites, including `wifi`: monarto DOES enable it (emitting an 800-byte `wisp/pucks.json` against welland's 2074), even though the gale puck fleet is welland-only — and **nothing delivers `pucks.json` to wisp at either site**: no Makefile target, no script in `~/local`, not served by nginx. The `letsencrypt` generator is enabled at **neither** site; it is invoked explicitly (`generate letsencrypt`) and welland's per-host DNS-01 certs are created from its output, while monarto manages its TLS certs with **certbot directly** — see *Let's Encrypt* below.
