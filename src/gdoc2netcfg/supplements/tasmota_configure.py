...
def configure_tasmota_device(
    ...
    # Apply drifted fields.  MqttPassword can never be read back, so it is never
    # in the drift list — but it MUST be pushed alongside any MqttUser change
    # (otherwise the new user authenticates with the old user's password and a
    # previously-working device drops off the broker on migration), and also when
